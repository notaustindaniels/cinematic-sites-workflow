...
- **IMAGERY (NEW — no stock photos, ever):** for every section/module that shows a photo (a `parallax-sections` background, a `stagger-grid` item, an image-bearing content section), write a **business-specific Flux image prompt** so a real image is generated for it — never a stock placeholder. Collect them in an `images[]` array: each entry `{ "id": "<short-slug>", "prompt": "<a vivid, photoreal Flux prompt grounded in THIS business — its subject, materials, geography — AND its BRAND PALETTE: translate brand-system.json's color_primary/color_secondary/color_accent + theme_direction + mood into concrete photographic COLOR LANGUAGE (the dominant temperature, light quality, and material tones that carry those hues) so the image shares the page's exact color world — e.g. a warm gold/cream palette -> 'warm golden-hour light, honey timber, cream-grey stone, deep amber glow'. NEVER a generic stock color look disconnected from the brand.>", "usage": "<which section/module + slot it fills, e.g. 'parallax:What We Build' or 'stagger:0'>" }`. Then **reference that id** in the module's image slot as `"image": "img:<id>"` (the assembler swaps `img:<id>` for the generated file). Write a DISTINCT prompt per image (variety — different rooms/vantages/details), all on-brand. Aim for 2–5 images total (the most image-forward sections).
- **TESTIMONIALS (social proof — the skill renders these too):** write **3–4** short, believable client testimonials grounded in THIS business — its service, geography, and customer type. Each `{ "quote": "<1–2 punchy sentences in the client's voice>", "author": "<plausible client name>", "role": "<role + place, e.g. 'Homeowner · Snowmass Village'>" }`. Collect them in a `testimonials[]` array. If `intake` provided real testimonials, use those verbatim instead of inventing. Never leave this empty for a maxed-out site.
...
- `testimonials` — array of `{ quote, author, role }` objects (3–4 believable, on-brand client testimonials; the site-build renders them as a social-proof section).
