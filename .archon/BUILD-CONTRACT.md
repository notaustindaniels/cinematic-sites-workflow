...
| `plan-step.ts` | `$A/<list>/plan.json` | (mutates on --mark-done) | per-mode | CLI: `--next <list>` prints first pending item JSON or `NONE`; `--all-done <list>` exit 0 iff no pending; `--mark-done <list> <id>` sets status=done. `<list>`∈{showcase,beforeafter,hero}. |
...
| SECTION marker | — | `$A/.cine-section` ("hero"\|"showcase") | — | `lib/util.ts cineSection()/sectionPaths()` resolve `$A/<sec>/` + `$A/scenes/<sec>-grid/` from this marker (default "showcase"). The grid scripts below (panel-flatten, compose-grid, door-swing, gen-flythrough, grid-verdict-passed) are section-aware via it; cinematic-site writes "hero" for the hero phase, "showcase" for the showcase phase (serialized). |
| `gen-hero-2frame.ts` | `$A/<sec>/grid-spec.json` + `$A/scenes/<sec>-grid/{p1,p2}.png` (sec from marker; hero) | hero → `$A/scenes/hero/hero-final.mp4` (else `$A/scenes/<sec>/<sec>-video.mp4`) | result URL | the HERO render — EXACTLY 2 panels → Seedance `--start-image p1 --end-image p2` (interpolates the A→B arrival). Guards: no timestamp/per-second beats; balanced phrase emphasis passed through (stripped if malformed). |
...
| `hero-director` (= `showcase-director`, marker=hero) | `$A/.cine-section`=hero, `$A/brand/brand-system.json`, `$A/plan.json` (hero hint) | `$A/hero/grid-spec.json` — EXACTLY 2 panels [start = exterior approach + closed single door; end = interior framed from WITHIN] | the hero REUSES the showcase Director in HERO MODE (Phase 0): a start+end pair, all lessons (zero-remnants, single-door, path) apply; rendered by gen-hero-2frame. `door-check` (marker=hero) sets the swing into `$A/hero/grid-spec.json`. |
...
`brand.md`(C.2) `plan.md`(C.3) `keyframe.md`(C.6) `grid.md`(grid/hero spec critic — showcase + hero)
